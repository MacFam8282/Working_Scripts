<# RemoteFetchAndServe.ps1
 ...
 - Optional read-only HTTP index server with streaming and correct headers.
...
function Start-RemoteHttpServer {
  <#
    ...
    - Correct headers, streaming, keep-alive disabled.
  ...
  while ($true) {
    ...
    try {
      ...
      if ($rel -ieq 'index.html') {
        $files = @()
        try { $files = Get-ChildItem -Path $Root -File -Recurse | Sort-Object FullName } catch {}
        $list = ($files | ForEach-Object {
          $href = ($_.FullName.Substring($Root.Length).TrimStart('\').Replace('\','/'))
          '<li><a href="./' + $href + '">' + $_.Name + '</a></li>'
        }) -join ''
        $html = "<!doctype html><html><head><meta charset='utf-8'><title>Index</title></head><body><h3>Files under $Root</h3><ul>$list</ul></body></html>"
        Send-Text -res $res -text $html -code 200 -mime 'text/html'
        continue
      }
...
'@
    ...
    $p = Start-Process -FilePath (Get-Command powershell.exe).Path -ArgumentList $argList -WindowStyle Hidden -PassThru
    Set-Content -Path $pidfile -Value $p.Id -Encoding ASCII -Force
...
function Stop-RemoteHttpServer {
  <#
    Stops the remote HTTP server fast and safely.
    - Reads PID from the known pidfile and kills that process.
    - Best-effort job/ACL/firewall cleanup.
    - Never blocks the local menu/terminal.
  #>
  ...
  $stopScript = {
    ...
    # Cleanup any leftover background jobs (old versions)
    ...
    # Clean ACL/firewall
    if ($Port -gt 0) {
      Remove-UrlAclForPort $Port
      Remove-FirewallRuleForPort $Port
    } else {
      foreach ($p in (8080,8000,8888,9090) + (49160..49180)) {
        Remove-UrlAclForPort $p
        Remove-FirewallRuleForPort $p
      }
    }
      ...
      switch ($choice) {
        '1' {
          try {
            ...
            if (-not $items -or @($items).Count -eq 0) { Write-Host "(empty)" }
            else { @($items) | Format-Table -AutoSize }
            Write-Host "✅ Task complete: View files"
        ...
        default {
          Write-Host "Invalid choice. Enter 1-6."
        }
      }
    }
  }
